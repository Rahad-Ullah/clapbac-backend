...
export const startAnnounceCron = (redis: any) => {
  // run every 5 minutes
  cron.schedule('*/5 * * * *', async () => {
    ...
    try {
      // Find audiences with ready scheduled announces
      ...
      for (const audience of audiences) {
        ...
        if (redis) {
          await redis.set(
            `announce:active:${audience}`,
            JSON.stringify(latest)
          );
        }
      }

      console.log('[Announce Cron] Completed for audiences:', audiences);
    } catch (err: any) {
      console.error('[Announce Cron] Error:', err.message);
    }
  });
};
